...
- (void) addFiltersToQueue {
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (colorInvert){
            ...
            [self addCIImageToCollectionView:colorInvert.outputImage withFilterTitle:NSLocalizedString(@"Color Cube", @"Color Cube Filter")];
        }
    }];
    // Bump Distortion
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *bump = [CIFilter filterWithName:@"CIBumpDistortion"];
                if (bump){
                    [bump setValue:sourceCIImage forKey:kCIInputImageKey];

                    
                    [bump setValue:[CIVector vectorWithX:200 Y:150] forKey:@"inputCenter"];
                    [bump setValue:[NSNumber numberWithFloat:100] forKey:@"inputRadius"];
                    [bump setValue:[NSNumber numberWithFloat:3.0] forKey:@"inputScale"];

            
            

                    //[bump setValue:bumpStart forKey:kCIInputCenterKey];
                    [self addCIImageToCollectionView:bump.outputImage withFilterTitle:NSLocalizedString(@"Bump", @"Bump Filter")];
        }
    }];
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *colorInvert = [CIFilter filterWithName:@"CIColorInvert"];
        CIFilter *bumpDistortion = [CIFilter filterWithName:@"CIBumpDistrotion"];
        
        if (colorInvert){
            [colorInvert setValue:sourceCIImage forKey:kCIInputImageKey];
            
            CIImage *result = colorInvert.outputImage;
            ...
            if (bumpDistortion){
                ...
                result = bumpDistortion.outputImage;
            }
